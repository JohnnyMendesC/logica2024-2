			            ...
			            se (verifNumdeVagaDigitado(vagaSelecionada))
			            	{
			            		se (processarEntrada(vagaSelecionada))
		            				{
		            					escreva("A vaga ", vagaSelecionada," está disponível. Deseja ocupa-lá? [S]/[N]")
		            					leia(ok)
		            						se (ok=="S" ou ok=="s" ou ok=="sim")
		            							{escreva("Você reservou a vaga ", vagaSelecionada," com sucesso.")}senao{escreva("Vaga não reservada, para reservar outra vaga selecione a opção 1")numerodeVagas[vagaSelecionada]=0}
