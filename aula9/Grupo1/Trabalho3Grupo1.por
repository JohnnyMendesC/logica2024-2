programa
{
	...
	funcao dados(inteiro opcao){
	    	...
	    	cadeia dadosPaciente[20][3]
	    	inteiro especialidade
	    	inteiro contadorTotalPacientesGeral
	    	cadeia nome, numero
	    	     
		para(inteiro l=0; l < 20 ; l++)
	     	{
			se(dadosPaciente[l][2] == "Pediatria"){l++}
...
deverá ser exibida a mensagem “Quarto Ocupado !” Para cada internação cadastrada deverá ser armazenado
em uma variável o número de pacientes internados e o total geral de internações para cálculo de faturamento. 
...
 Função para preencher dados do paciente. (nome, telefone e especialidade da consulta). 
 Uma função para verificação de quartos ocupados
 Função para verificar se o número do quarto foi digitado corretamente. 
 Listagem de quartos
 Verificação do número se está correto entre 1 e 20. 
 Listagem de vagas
 ...
 * @POSICAO-CURSOR = 806; 
 * @PONTOS-DE-PARADA = ;
 * @SIMBOLOS-INSPECIONADOS = {dadosPaciente, 60, 13, 13};
